...
function __pf_set_separator -d "Check for Powerline font and set separator"
    ...
    #if test -n (locate powerline)
    if test (locate powerline | wc -l) -gt 0
...
function __ken_time_prompt -d "Write out the time at end of prompt"
#        set --global pf_user_status_bg
#        set --global pf_user_status_text $pf_text_light
#        set pf_user_status_bg $pf_color_user
...
function __ken_acpi_prompt -d "Check and write battery life"
